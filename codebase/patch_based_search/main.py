...
def train(env: PathTraversalEnv, episodes=2000):
    ...
    rewards = []
    prev_dead_end = False
    options = {"start_from_root_seed": True}
    
    for ep in range(episodes):
        if prev_dead_end:
            options["start_from_root_seed"] = False
        ...
        if env.render_mode == "rgb_array" and ep%100 == 0:
            frame = env.render()
            im = Image.fromarray(frame)
            im.save(f"{os.getcwd()}/episodes_results/ep_{ep+1}_coverage_{info.get('coverage', 0)*100:.3f}.png")
